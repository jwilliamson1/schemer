...
(define (adjoin-set! set keys value)
  (let ((key (car keys)))
    (cond ((null? set) (make-tree-node (make-entry key value)))
          ((eq? key (caar set))
           (if (null? (cdr keys))
               (set-cdr! (car set) (cdr (make-entry key value)))
               (set-cdr! (car set) (adjoin-set! (car set) (cdr keys) value))
                      )
           set)
          ((< key (entry set))
...
(define (insert-tree! value table . keys)  
...
t1
(insert-tree! 'mom t1 42)
t1
;overwrite root test
(insert-tree! 'dad t1 42)
(insert-tree! 'son t1 22)
(insert-tree! 'niece t1 32)
(insert-tree! 'nephew t1 12)
(insert-tree! 'cousin t1 12)
(insert-tree! 'cousina t1 9)
(insert-tree! 'cousin-twice-removed t1 12 5)
